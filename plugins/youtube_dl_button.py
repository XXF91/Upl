...
async def youtube_dl_call_back(bot, update):
    ...
    if e_response:
        ...
        message_id=update.message.id, text="**خطأ  :  تحميل فاشل ⚠️**")
        return
    if not t_response:
        asyncio.create_task(clendir(tmp_directory_for_each_user))
        await bot.edit_message_text(chat_id=update.message.chat.id,
        text="خطأ  : لم يتم إيجد الملف 😑", message_id=update.message.id)
        return
    file_size, file_location = await get_flocation(download_directory, youtube_dl_ext)
    if file_size == 0:
        await update.message.edit(text="خطأ  : لم يتم إيجد الملف 🙁")
        asyncio.create_task(clendir(tmp_directory_for_each_user))
        return
